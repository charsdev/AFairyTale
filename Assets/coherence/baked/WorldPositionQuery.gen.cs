namespace Coherence.Generated
{
	...
	public struct WorldPositionQuery : ICoherenceComponentData
	{
		...
		public uint DiffWith(ICoherenceComponentData data)
		{
			...
			if (Diffing.HasNoticeableDifference(position, newData.position, _position_Epsilon)) {
				mask |= 0b00000000000000000000000000000001;
			}
			if (Diffing.HasNoticeableDifference(radius, newData.radius, _radius_Epsilon)) {
		...
		public static void Serialize(WorldPositionQuery data, uint mask, IOutProtocolBitStream bitStream)
		{
			...
			{
				Coherence.Toolkit.Bounds.Check(data.position.x, _position_Min, _position_Max, "WorldPositionQuery.position.x");

				Coherence.Toolkit.Bounds.Check(data.position.y, _position_Min, _position_Max, "WorldPositionQuery.position.y");

				Coherence.Toolkit.Bounds.Check(data.position.z, _position_Min, _position_Max, "WorldPositionQuery.position.z");
			...
			if (bitStream.WriteMask((mask & 0x01) != 0))
			{
				Coherence.Toolkit.Bounds.Check(data.radius, _radius_Min, _radius_Max, "WorldPositionQuery.radius");
